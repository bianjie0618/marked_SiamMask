...
parser.add_argument('--clip', default=10.0, type=float,
                    help='gradient clip value')  # 控制梯度爆炸的一种解决办法；https://blog.csdn.net/u010814042/article/details/76154391
...
def build_data_loader(cfg):
    logger = logging.getLogger('global') # 实例化一个name=global的记录器，全局可用(如果有，则无需创建)
...
def main():
    ...
    logger.info("config \n{}".format(json.dumps(cfg, indent=4)))  # 转变成json格式的文件，缩进4格
    ...
    dist_model = torch.nn.DataParallel(model, list(range(torch.cuda.device_count()))).cuda()  # 多GPU训练

    if args.resume and args.start_epoch != 0:  # 这是在干啥？蒙蔽了！！！！！
        model.features.unfix((args.start_epoch - 1) / args.epochs)

    optimizer, lr_scheduler = build_opt_lr(model, cfg, args, args.start_epoch)   # 如何构建优化器和学习策略？？？
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        if iter % num_per_epoch == 0 and iter != 0:
            for idx, pg in enumerate(optimizer.param_groups):   # 如何理解pytorch里面的参数组param_groups？？？
        ...
        loss.backward()
        # model里的module是怎么出来的？
        if cfg['clip']['split']:  # 以下是梯度爆炸的解决方案（有分而治之和统一解决两种方案）
...
def save_checkpoint(state, is_best, filename='checkpoint.pth', best_file='model_best.pth'):
    ...
    if is_best:
        shutil.copyfile(filename, best_file) # 复制一份文件
