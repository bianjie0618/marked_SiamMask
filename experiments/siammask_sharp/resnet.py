...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, layer4=False, layer3=False):
        ...
        self.feature_size = 128 * block.expansion #这里显示输出的channel，由resnet可知输出的确实是128x4

        if layer3:  # 由于height、width都不在往下降了，选择的方案是dialation=2，stride=1、paddding=(2,2),从而保证尺度不降
            self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2) # 15x15, 7x7
            self.feature_size = (256 + 128) * block.expansion # 为什么是这样？？？
        else:
