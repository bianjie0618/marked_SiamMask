...
class ResDownS(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(3) < 20:
            ...
            r = -4
            x = x[:, :, l:r, l:r]   # 为什么要这么做？？？
            # search featuremap和exemplar featuremap的大小减小了8. 理解了这一点为后面的反向refine操作打好了铺垫
        return x


class ResDown(MultiStageFeature):  # 这里我们定义的类叫做ResDown
    def __init__(self, pretrain=False):
        super(ResDown, self).__init__()
        self.features = resnet50(layer3=True, layer4=False) # 从resnet50中选取一些结构，只选取了前三个大块
        if pretrain:
...
class UP(RPN):
    def __init__(self, anchor_num=5, feature_in=256, feature_out=256):
        ...
        #分类输出，正类还是负类，故*2；定位anchor需要两对坐标四个输出，故*3
...
class Refine(nn.Module):
    def __init__(self):
        ...
        self.v0 = nn.Sequential(nn.Conv2d(64, 16, 3, padding=1), nn.ReLU(),
                           nn.Conv2d(16, 4, 3, padding=1),nn.ReLU())   # 这里应该是(16,8,3)
        ...
        # 反向卷积。如果正向卷积搞懂了，则反向卷积也就是公式的变换；
        # 注意，反向卷积也是卷积，并不是逆卷积的意思，只是把形状(C H W)逆推出来。
    ...
    def forward(self, f, corr_feature, pos=None, test=False):
        ...
        out = self.deconv(p3)
        out = self.post0(F.upsample(self.h2(out) + self.v2(p2), size=(31, 31)))  # 上采样扩重充空间维度，post卷积减少通道数量
