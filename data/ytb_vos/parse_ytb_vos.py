...
def polys_to_boxes(polys):
    ...
    for i in range(len(polys)):  # poly的shape应该是(c,n),c代表多边形的个数，n代表该多边形顶点个数的2倍。
        poly = polys[i]
        x0 = min(min(p[::2]) for p in poly)  # 这种写法对吗？？？
...
def convert_ytb_vos(data_dir, out_dir):
    ...
    for data_set, ann_dir in zip(sets, ann_dirs):
        ...
        json_ann = json.load(open(os.path.join(ann_dir, '../meta.json')))   # 这是原始数据meta.json
        for vid, video in enumerate(json_ann['videos']):
            ...
            for frame in frames:
                ...
                img = cv2.imread(fullname, 0)   # 读入灰度图像
                h, w = img.shape[:2]
                ...
                for instanceId in np.unique(img):
                    if instanceId == 0:
                        continue
                    instanceObj = Instance(img, instanceId)   # 实例化一个Instance
                    instanceObj_dict = instanceObj.toDict()
                    mask = (img == instanceId).astype(np.uint8)
                    _, contour, _ = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                    # 返回的contour是一个list，里面的每一个元素是一个具体的contour
                    # reshape(-1)把数据转换成一维了
                    polygons = [c.reshape(-1).tolist() for c in contour]   # 即便是一个分割对象，也可能有多个多边形(遮挡引起)
        ...
        val_dict = dict(items[3000:])    # 可以将每一个tuple转变成一个键值对
