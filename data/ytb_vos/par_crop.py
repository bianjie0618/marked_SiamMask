...
def crop_video(video, v, crop_path, data_path, instanc_size):
    video_crop_base_path = join(crop_path, video)   # video参数表示某一视频的名字
    ...
    # ytb_vos是视频序列的集合，每个视频序列里有若干个分割对象，每个分割对象都有一个id
    # 当字典转化成list时，list中只有key值，没有对应的value值
    for trackid, o in enumerate(list(v)):   # v里面有多少个元素，关键在于该视频序有多少个o; 这里的trackid仅表示
        obj = v[o]   # o 表示obj_id; obj_id 同时也是annotations下该对象的mask的灰度值。
        for frame in obj:
            file_name = frame['file_name']
            ann_path = join(anno_base_path, file_name+'.png')
            img_path = join(img_base_path, file_name+'.jpg')
            im = cv2.imread(img_path)
            label = cv2.imread(ann_path, 0)   # 0是一个flag标志位，代表以灰度模式加载图像：cv.IMREAD_GRAYSCALE
