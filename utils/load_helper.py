...
def check_keys(model, pretrained_state_dict):
    ...
    used_pretrained_keys = model_keys & ckpt_keys    # 集合运算中的交集运算
    unused_pretrained_keys = ckpt_keys - model_keys  # 集合运算中的差集运算
...
def load_pretrain(model, pretrained_path):
    ...
    if not torch.cuda.is_available():
        # 要了解torch.load()的用法，尤其是关于map_location这一参数
    ...
    try:
        check_keys(model, pretrained_dict)  # 如果之前有过加载，就无需再次加载预训练模型
    except:
    ...
    model.load_state_dict(pretrained_dict, strict=False)  # 为什么要加上features才能匹配？？？&&&&&&&& 注意本行 &&&&&&&&&&&&&
    return model
