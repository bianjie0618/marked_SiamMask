...
Corner = namedtuple('Corner', 'x1 y1 x2 y2')  # 命名元祖，元祖的name是Corner
BBox = Corner
...
def aug_apply(bbox, param, shape, inv=False, rd=False):
    ...
    if not inv:
        ...
        if 'shift' in param:
            ...
            tx = max(-x1, min(imw - 1 - x2, tx))   # 移动bbox一个差不多的距离
...
def IoU(rect1, rect2):
    ...
    xx1 = np.maximum(tx1, x1)   # 逐个比较；画个图看看单个anchorIoU。很容易就明白了；这里面计算的是全部anchors的IOU
