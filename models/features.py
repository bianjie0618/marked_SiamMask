...
class Features(nn.Module):
    ...
    def param_groups(self, start_lr, feature_mult=1):  # 过滤可以训练的参数
        params = filter(lambda x:x.requires_grad, self.parameters())
        params = [{'params': params, 'lr': start_lr * feature_mult}]
        return params

    def load_model(self, f='pretrain.model'):  # 如何载入模型
        with open(f) as f:
            pretrained_dict = torch.load(f)
            model_dict = self.state_dict()  # state_dict()包含了所有合法的参数名，所以如果不在其中的参数名，一定不是合法的参数名
...
class MultiStageFeature(Features):
    ...
    def train_layers(self): # 分段训练，逐步加深训练层数
        return self.layers[:self.train_num]

    def unlock(self): # 解冻冰封的层，进行训练
